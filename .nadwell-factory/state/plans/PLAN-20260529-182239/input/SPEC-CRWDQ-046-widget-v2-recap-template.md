...
depends_on: [SPEC-CRWDQ-022, SPEC-CRWDQ-023, SPEC-CRWDQ-034, SPEC-CRWDQ-064]
...
| Source files | `modules/widget-v2/src/render/GameStateStore.ts`, `PlannedStateActivator.ts`, `ProgramSlotResolver.ts`, `TransitionExecutor.ts`, `DwellTimer.ts` (consumed from SPEC-CRWDQ-023); `AssetManifestStore.ts` (consumed from SPEC-CRWDQ-064); `FixtureListStore.ts` (consumed from SPEC-CRWDQ-034) |
...
- `GameStatePayload` (verified `src/wire/types.ts:135-145`) carries NO team identity. Team **names** come from the player's cached `FixtureList` record, joined on `event_id` (see the RESOLVED note below).

> **RESOLVED — team identity comes from the FixtureList, not GameState
> (backend code cross-check).** Verified against the `crowdaq-backend`
> source: `GameStatePayload` (`src/wire/types.ts:135-145`) carries exactly
> `game_id`, `sport`, `home_score`, `away_score`, `period`, `clock`,
> `signals`, `badges`, `sport_context` — and NO team identity of any kind
> (no `home_team_id` / `away_team_id`, no team names). There is no
> game-state table; `GameState` lives only in an in-memory per-process
> cache server-side (`src/delivery/cache/game-state.ts`). Team **names**
> are carried only on the fixture record — the `fixtures` catalog table
> (`src/catalog/schema.ts` `home_team` / `away_team`) and the domain
> `FixtureListEntry` (`homeTeam` / `awayTeam`,
> `src/domain/fixtures/fixture-list-projection.ts:29-38`).
>
> Resolution: the recap template shall obtain team identity by joining
> `gameState.game_id` (which is the canonical `event_id`) against the
> player's cached `FixtureList` record for the same `event_id`, reading
> `homeTeam` / `awayTeam` display-name strings from that
> `FixtureListEntry`. The player shall NOT read team fields off
> `GameState`. There is no `team_id` on any player-facing wire payload,
> so per-team logos keyed by `team_id` (D-GRH-08) cannot be resolved for
> the recap — the recap renders team **names** (text), and the
> `AssetManifest` join is limited to the sport/league badge (keyed by
> `sport` + league, both available). The DOM `data-team-id` attribute and
> the per-team logo `<img>` are therefore dropped from the recap
> composition; see the updated DOM shape and activation flow below.
...
export interface RecapContext {
  ...
  /**
   * The player's cached FixtureList (SPEC-CRWDQ-034 FixtureListStore).
   * Recap team identity is joined from here by event_id — `GameState`
   * carries no team fields (verified crowdaq-backend src/wire/types.ts).
   */
  fixtureListStore: FixtureListStore;
  assetManifestStore: AssetManifestStore;  // SPEC-CRWDQ-064 — sport/league badge resolution
  /**
...
`ResolvedTheme`, `PendingPreferenceApply`, `ProgramSlotPayload`, and `GameStateStore` are defined by SPEC-CRWDQ-023 and consumed verbatim. `AssetManifestStore` is owned by SPEC-CRWDQ-064. `FixtureListStore` is owned by SPEC-CRWDQ-034 — the recap consumes it read-only (`resolve(eventId)`) to obtain team names.
...
<section class="crowdaq-recap" data-theme data-game-id data-winner="home|away|draw">
  ...
  <div class="cdq-final-score">
    <div class="cdq-team cdq-team-home">
      <span class="cdq-team-name">HOME TEAM NAME</span>
      <span class="cdq-team-score">N</span>
    </div>
    <div class="cdq-team cdq-team-away">
      <span class="cdq-team-name">AWAY TEAM NAME</span>
...
There is no per-team logo `<img>` and no `data-team-id` attribute: no
player-facing wire payload carries a `team_id` (verified against
`crowdaq-backend` `src/wire/types.ts`), so a per-team logo cannot be
resolved. Team names render as text, joined from the cached `FixtureList`
(see the RESOLVED team-identity note above). The header still renders a
sport/league badge — that asset is keyed by `sport` + league, both
available.
...
4. **Compose final composition.** Read `home_score` / `away_score` from `gameState`. Obtain team **names** by joining `gameState.game_id` (the canonical `event_id`) against `fixtureListStore.resolve(gameState.game_id)` and reading `homeTeam` / `awayTeam` from the returned `FixtureListEntry` (see the RESOLVED team-identity note). `GameState` carries no team fields. Resolve the sport/league badge from `AssetManifestStore` (keyed by `sport` + league); there is no per-team logo. Compute `winner = home_score > away_score ? "home" : away_score > home_score ? "away" : "draw"`.
...
- `FixtureList` miss on `event_id` (no cached fixture record for the recap's game) → the team renders a sport-neutral `"Home"` / `"Away"` placeholder label, no error. The final score alone is still useful.
- Badge-asset cache miss → empty header badge slot; the sport/league text is still shown where available.
...
| `AssetManifestStore` (SPEC-CRWDQ-064) | 1 in-process | Real instance, pre-seeded with sport/league badge assets; `AssetFetcher` substituted per SPEC-CRWDQ-064. |
| `FixtureListStore` (SPEC-CRWDQ-034) | 1 in-process | Real instance, pre-seeded with the game's fixture record; driven via frame injection. |
| Journal sink | 2 local-substitutable | In-memory. |

Test cases:

- Happy recap: `GameState{status: final, home_score: 3, away_score: 1}` + a cached `FixtureListEntry` for the game's `event_id` + a `headlineMoment{kind:'goal'}` in context → DOM shows the FULL TIME label, `data-winner="home"`, scores 3/1, team names from the fixture record, and one `.cdq-headline-moment` block with `data-moment-kind="goal"`.
...
- FixtureList miss: `fixtureListStore.resolve(gameState.game_id)` → null → teams render `"Home"` / `"Away"` placeholder labels, no error.
- Badge-asset cache miss: `assetManifestStore.get(<sport/league badge id>)` → null → header badge slot empty, no error.
...
- team name join — `GameState` carries no team identity; the recap reads `homeTeam` / `awayTeam` from the cached `FixtureListEntry` (SPEC-CRWDQ-034 `FixtureListStore`) resolved by `gameState.game_id` (the canonical `event_id`).

## Acceptance Criteria

- [ ] `RecapTemplate.mount(host, ctx)` renders `<section class="crowdaq-recap" data-theme data-game-id data-winner="home|away|draw">` with a header (FULL TIME label + sport_context), a final-score block (home/away team names + scores — no per-team logo, no `data-team-id`), and an optional single headline-moment block.
- [ ] `data-winner` is derived from `gameState.home_score` vs `gameState.away_score` — `home`, `away`, or `draw`; not read from the wire.
- [ ] Team names are obtained by joining `gameState.game_id` (the canonical `event_id`) against `fixtureListStore.resolve(...)` and reading `homeTeam` / `awayTeam`; `GameState` carries no team identity (verified `crowdaq-backend` `src/wire/types.ts:135-145`). A `FixtureList` miss on `event_id` falls back to `"Home"` / `"Away"` placeholder labels.
...
- [ ] No per-team logo is rendered — no player-facing wire payload carries a `team_id` (verified `crowdaq-backend` `src/wire/types.ts`); the `AssetManifest` join is limited to the sport/league badge, and a badge-cache miss leaves the header badge slot empty without error.
...
- [ ] Tests cover happy path, draw, null headline moment, FixtureList miss, badge cache miss, null primary, no-gamestate, premature, late event, re-push order buffering, dwell boundary, supersede, pending apply.
- [ ] No mocks of the shared orchestration, `GameStateStore`, or `FixtureListStore` (INV-FACTORY-16); only the asset URL fetch boundary and the clock are substituted (INV-FACTORY-17).
