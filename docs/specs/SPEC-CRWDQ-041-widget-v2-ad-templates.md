...
// AdSlot mirrors SPEC-CRWDQ-017 `AdSlotPayload` (the wire type) — consumed, not independently owned.
...
- Ad asset cache miss: `assetManifestStore.get(adSlot.ad_ref)` → null → journal `ad_asset_cache_miss`, `ensure(adSlot.ad_ref)` called once; composite falls back to non-ad mode (mounts `MultiGameTemplate` or `FixturesTemplate` directly into the original host without the composite shell).
...
- [ ] An `AssetManifestStore.get(adSlot.ad_ref)` miss journals `ad_asset_cache_miss`, calls `ensure(adSlot.ad_ref)` to warm the cache, and falls back to the non-ad template (no composite shell, no blank ad panel); content is preserved per D-GRH-16.
