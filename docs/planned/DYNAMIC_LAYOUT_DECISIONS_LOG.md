...
**Schema addition:** `BarPreferences` gains `state` (state/province code) and `city` (city slug) fields to support the phase-1 rule scope set below. The `ConfigPush` payload in D-GRH-60 is updated correspondingly.
